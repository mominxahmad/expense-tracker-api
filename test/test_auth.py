...
@pytest.mark.asyncio
async def test_confirm_and_get_user(test_user):
    username = "test_user"
    password = "testing123"
    async with TestingSessionLocal() as db:
        check = await confirm_and_get_user(username, password, db)
    assert check.id == 1
...
def test_authenticate_current_user():
    encode = {
        "sub": "test_user",
        "id": 1,
        "role": "admin"
    ...
    decoded_token = authenticate_current_user(token)
...
def test_authenticate_current_user_FAILURE():
    encode = {
        "role": "admin"
    }
    token = jwt.encode(encode, SECRET_KEY, ALGORITHM)
    with pytest.raises(HTTPException) as excinfo:
        authenticate_current_user(token=token)
